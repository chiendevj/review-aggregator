...
module.exports = {
  async up(queryInterface) {
    ...
    for (const productId of Object.keys(mockReviews)) {
      const sources = mockReviews[productId];

      for (const vendor of Object.keys(sources)) {
        for (const r of sources[vendor]) {
          rows.push({
    ...
    return queryInterface.bulkInsert("reviews", rows);
  },
